  ...
  var _ref = _asyncToGenerator(function*(data) {
    ...
    if (filePath.endsWith(PACKAGE_JSON)) {
      ...
      try {
        ...
        if (fileData.name) {
          const relativeFilePath = (_path || _load_path()).default.relative(
            rootDir,
            filePath
          );
          id = fileData.name;
        ...
        dependencies = Array.from(
          data.dependencyExtractor // $FlowFixMe
            ? require(data.dependencyExtractor).extract(
                content,
                (_dependencyExtractor || _load_dependencyExtractor()).extract
              )
            : (_dependencyExtractor || _load_dependencyExtractor()).extract(
                content
              )
      ...
      if (id) {
        ...
        module = [
          relativeFilePath,
          (_constants || _load_constants()).default.MODULE
        ];
      }
    }
...
var _dependencyExtractor;

function _load_dependencyExtractor() {
  return (_dependencyExtractor = _interopRequireWildcard(
